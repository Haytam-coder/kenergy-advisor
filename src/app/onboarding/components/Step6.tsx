...
export default function Step6({ data, onChange, onSubmit, onBack, submitting }: Props) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-inter-var)', fontWeight: 400, fontSize: '44px', color: 'var(--text)', marginBottom: '6px', lineHeight: 1 }}>Fast fertig.</h2>
      <p style={{ fontFamily: 'var(--font-inter-var)', fontSize: '16px', lineHeight: 1.7, color: 'var(--muted)', marginBottom: '28px' }}>Noch ein paar letzte Details.</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ label: 'Unter 5 J.', value: 'unter5' as const }, { label: '5–10 J.', value: '5bis10' as const }, { label: 'Über 10 J.', value: 'ueber10' as const }].map((opt) => (
              <button key={opt.value} onClick={() => onChange({ gerateAlter: opt.value })} className={sc(data.gerateAlter === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '12px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ label: '✅ Ja, alles', value: 'ja' as const }, { label: '🔆 Teilweise', value: 'teilweise' as const }, { label: '❌ Nein', value: 'nein' as const }].map((opt) => (
              <button key={opt.value} onClick={() => onChange({ ledBeleuchtung: opt.value })} className={sc(data.ledBeleuchtung === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '12px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ label: 'Unter 2 J.', value: 'unter2' as const }, { label: '2–5 J.', value: '2bis5' as const }, { label: 'Langfristig', value: 'langfristig' as const }].map((opt) => (
              <button key={opt.value} onClick={() => onChange({ wohndauer: opt.value })} className={sc(data.wohndauer === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '12px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {[{ label: 'Ja', value: 'ja' as const }, { label: 'Nein', value: 'nein' as const }, { label: 'Weiß nicht', value: 'weiss_nicht' as const }].map((opt) => (
              <button key={opt.value} onClick={() => onChange({ foerderungenBeantragt: opt.value })} className={sc(data.foerderungenBeantragt === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '12px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
      </div>
