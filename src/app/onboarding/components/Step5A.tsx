...
export default function Step5A({ data, onChange, onNext, onBack }: Props) {
  ...
  return (
    <div>
      <h2 style={{ fontFamily: 'var(--font-inter-var)', fontWeight: 400, fontSize: '44px', color: 'var(--text)', marginBottom: '6px', lineHeight: 1 }}>Dein Haus</h2>
      <p style={{ fontFamily: 'var(--font-inter-var)', fontSize: '16px', lineHeight: 1.7, color: 'var(--muted)', marginBottom: '28px' }}>Details für Eigentümer eines Hauses.</p>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{ label: '☀️ Süden', value: 'sued' as const }, { label: '↔️ Ost/West', value: 'ost_west' as const }, { label: '🌑 Norden', value: 'nord' as const }, { label: '❓ Weiß nicht', value: 'weiss_nicht' as const }].map((opt) => (
              <button key={opt.value} onClick={() => onChange({ dachausrichtung: opt.value })} className={sc(data.dachausrichtung === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '14px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {[{ label: '✅ Ja', value: true }, { label: '❌ Nein', value: false }].map((opt) => (
              <button key={String(opt.value)} onClick={() => onChange({ solarVorhanden: opt.value })} className={sc(data.solarVorhanden === opt.value)} style={{ fontFamily: 'var(--font-inter-var)', fontSize: '14px' }}>{opt.label}</button>
            ))}
          </div>
        </div>
        <div>
